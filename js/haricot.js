window.haricot = window.haricot || {};

( function() {

	// Bail if we don't have the JSON, which is passed in via `wp_localize_script()`.
	if ( _.isUndefined( haricot_data ) ) {
		return;
	}

	/**
	 * Our global object.  The `haricot` object is just a wrapper to house everything
	...
	api.templates.register_manager = function( type ) {
		...
		this.managers[ type ] = wp.template( 'haricot-manager-' + type );
	};
	...
	api.templates.register_section = function( type ) {
		...
		this.sections[ type ] = wp.template( 'haricot-section-' + type );
	};
	...
	api.templates.register_control = function( type ) {
		...
		this.controls[ type ] = wp.template( 'haricot-control-' + type );
	};
	...
	api.render = function() {
		...
		_.each( haricot_data.managers, function( data ) {
			...
			var metabox = document.getElementById( 'haricot-ui-' + manager.get( 'name' ) );

			// Add the `.haricot-ui` class to the meta box.
			metabox.className += ' haricot-ui';
	...
	// Nav template.
	var nav_template = wp.template( 'haricot-nav' );
	 ...
	 * `haricot.views.register_manager()` function.
	 *
	 * @since  1.0.0
	 * @access public
	 * @var    object
	...
	api.views.managers[ 'default' ] = Backbone.View.extend( {
		...
		attributes : function() {
			return {
				'id'    : 'haricot-manager-' + this.model.get( 'name' ),
				'class' : 'haricot-manager haricot-manager-' + this.model.get( 'type' )
			};
		},
		...
		subview_render : function() {
			...
			sections.forEach( function( section, i ) {
				...
				document.querySelector( '#haricot-ui-' + section.get( 'manager' ) + ' .haricot-nav'     ).appendChild( nav_view.render().el     );
				...
				document.querySelector( '#haricot-ui-' + section.get( 'manager' ) + ' .haricot-content' ).appendChild( view.render().el );
			...
			_.each( this.model.get( 'controls' ), function( data ) {
				...
				// Render the view.
				document.getElementById( 'haricot-' + control.get( 'manager' ) + '-section-' + control.get( 'section' ) ).appendChild( view.render().el );
	 ...
	 * `haricot.views.register_section()` function.
	 *
	 * @since  1.0.0
	 * @access public
	 * @var    object
	...
	api.views.sections[ 'default' ] = Backbone.View.extend( {
		...
		attributes : function() {
			return {
				'id'          : 'haricot-' + this.model.get( 'manager' ) + '-section-' + this.model.get( 'name' ),
				'class'       : 'haricot-section haricot-section-' + this.model.get( 'type' ),
	 ...
	 * `haricot.views.register_control()` function.
	 *
	 * @since  1.0.0
	 * @access public
	 * @var    object
	...
	api.views.register_control( 'color', {
		...
		ready : function() {
			...
			jQuery( this.$el ).find( '.haricot-color-picker' ).wpColorPicker( options );
		}
	} );
	...
	api.views.register_control( 'image', {
		...
		// Adds custom events.
		events : {
			'click .haricot-add-media'    : 'showmodal',
			'click .haricot-change-media' : 'showmodal',
			'click .haricot-remove-media' : 'removemedia'
		},
